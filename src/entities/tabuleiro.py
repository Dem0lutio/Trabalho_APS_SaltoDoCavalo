...
class Tabuleiro():
    ...
    def iniciar_partida(self, jogadores: list[str]) -> None:
        ...
        self.posicoes.posicoes_iniciais()

    def posicoes_iniciais(self):
        self.posicoes[4][4].bloqueada(True)
        self.posicoes[4][4].ocupada('CAVALO BRANCO')
        self.posicoes[0][0].bloqueada(True)
        self.posicoes[4][4].ocupada('CAVALO PRETO')

        if self._jogador_local.cor == 'BRANCO':
            self._jogador_local.posicao_atual(self.posicoes[4][4])
            self._jogador_remoto.posicao_atual(self.posicoes[0][0])
        else:
            self._jogador_local.posicao_atual(self.posicoes[0][0])
            self._jogador_remoto.posicao_atual(self.posicoes[4][4])
    ...
    def selecionar_posicao(self, linha: int, coluna: int) -> Union[Dict, None]:
        ...
        if status_partida == 'AGUARDANDO SELECAO CAVALO':
    ...
    def selecionar_posicao_destino(self, posicao_selecionada: Posicao) -> Union[Dict, None]:
        ...
        if not self.posicao_bloqueada(posicao_destino.x, posicao_destino.y):
            ...
            if posicao_destino.posicao_alcancavel(posicao_origem):
                ...
                if self.verifica_vencedor():
                    ...
                    if jogador_atual == self._jogador_local:
                        self.status_partida = 'AGUARDANDO SELECAO CAVALO'
                    else:
                        self.status_partida = 'AGUARDANDO JOGADOR REMOTO'
    ...
    def receber_jogada(self, jogada: Dict) -> None:
        if jogada['tipo'] == "jogada":
            self.selecionar_posicao_origem(jogada["linha_origem"], jogada["coluna_origem"])
            self.selecionar_posicao_destino(jogada["linha_destino"], jogada["coluna_destino"])
    ...
    def posicao_bloqueada(self, linha: int, coluna: int):
        return self.posicoes[linha, coluna].bloqueada
